...
(()=>{
  ...
  async function loadRemoteContent(){
    ...
    materials.splice(0,materials.length,...m.map(x=>({id:x.id,type:x.kind,unit:"모든 분반 공유",title:x.title,desc:x.description,pages:x.page_count,date:new Date(x.created_at).toLocaleDateString("ko-KR"),color:x.kind==='slide'?"green":"yellow",filePath:x.file_path,teacherId:x.teacher_id})));
  ...
  async function deleteMaterial(material,button){
    ...
    try{const {error}=await db.from("materials").delete().eq("id",material.id).eq("teacher_id",remote.user.id);if(error)throw error;if(material.filePath){const {error:fileError}=await db.storage.from("class-materials").remove([material.filePath]);if(fileError)console.warn("자료 파일 정리 실패",fileError)}await loadRemoteContent();toast("수업 자료를 삭제했습니다.")}catch(error){button.disabled=false;button.textContent="삭제";toast("자료를 삭제하지 못했습니다.");console.error(error)}
  }
  ...
  openViewer=async function(id){
    const loadId=++documentLoadId;remoteDocument?.viewer?.destroy();remoteDocument=null;document.querySelector("#documentCanvas")?.remove();document.querySelector("#pptxCanvas")?.remove();appOpenViewer(id);const material=materials.find(x=>x.id===id);if(!material?.filePath)return;
