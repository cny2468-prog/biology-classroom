...
(()=>{
  ...
  function renderTeacherDirectory(){
    ...
    const list=teacherStudents.filter(student=>!q||student.display_name.includes(q)).sort((a,b)=>compareKorean(a.display_name,b.display_name));
    document.querySelector("#teacherStudentCount").textContent=teacherStudents.length;
    const groups=new Map();
    for(const student of list){const classNames=student.classNames.length?[...new Set(student.classNames)].sort(compareKorean):["분반 미등록"];for(const className of classNames){if(!groups.has(className))groups.set(className,[]);groups.get(className).push(student)}}
