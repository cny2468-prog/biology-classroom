...
(()=>{
  ...
  function materialForm(){modalForm("전체 공유 수업 자료 올리기",'<p class="shared-notice">이 자료는 모든 분반 학생에게 공개됩니다.</p><label>자료 종류<select name="kind"><option value="slide">수업 슬라이드</option><option value="worksheet">학습지</option></select></label><label>제목<input name="title" required></label><label>설명<textarea name="description"></textarea></label><label>파일<input name="file" type="file" required accept=".pdf,.ppt,.pptx,image/*"></label>',async fd=>{const file=fd.get("file"),safe=`${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"_")}`,path=`shared/${safe}`;const {error:up}=await db.storage.from("class-materials").upload(path,file);if(up)throw up;const {error}=await db.from("materials").insert({class_id:null,teacher_id:remote.user.id,kind:fd.get("kind"),title:fd.get("title"),description:fd.get("description"),file_path:path});if(error)throw error})}
