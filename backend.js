...
(()=>{
  ...
  function decorateTeacherMaterials(){
    ...
    document.querySelectorAll("#materialGrid [data-open-material]").forEach(open=>{const material=materials.find(x=>x.id===open.dataset.openMaterial);if(!material||material.teacherId!==remote.user.id||open.parentElement.querySelector("[data-delete-material]"))return;const button=document.createElement("button");button.type="button";button.className="material-delete";button.dataset.deleteMaterial=material.id;button.textContent="삭제";button.onclick=event=>{event.stopPropagation();deleteMaterial(material,button)};open.before(button)});
  }
  ...
  function materialForm(){modalForm("전체 공유 수업 자료 올리기",'<p class="shared-notice">PDF와 PPTX는 슬라이드별로 자동 표시됩니다. 구형 PPT 파일은 PowerPoint에서 PPTX로 저장한 뒤 올려 주세요.</p><label>자료 종류<select name="kind"><option value="slide">수업 슬라이드</option><option value="worksheet">학습지</option></select></label><label>제목<input name="title" required></label><label>설명<textarea name="description"></textarea></label><label>PDF, PPTX 또는 이미지<input name="file" type="file" required accept=".pdf,.pptx,application/pdf,application/vnd.openxmlformats-officedocument.presentationml.presentation,image/png,image/jpeg,image/webp"></label>',async fd=>{const file=fd.get("file");if(!(file instanceof File)||!file.size)throw Error("올릴 PDF, PPTX 또는 이미지 파일을 선택해 주세요.");if(/\.ppt$/i.test(file.name))throw Error("구형 PPT 파일은 바로 표시할 수 없습니다. PowerPoint에서 PPTX로 다시 저장한 뒤 올려 주세요.");let pageCount=1;const buffer=await file.arrayBuffer();if(/\.pdf$/i.test(file.name)||file.type==='application/pdf'){if(!window.pdfjsLib)throw Error("PDF 기능을 불러오지 못했습니다. 페이지를 새로고침한 뒤 다시 시도해 주세요.");const pdf=await window.pdfjsLib.getDocument({data:buffer.slice(0)}).promise;pageCount=pdf.numPages;await pdf.destroy()}else if(/\.pptx$/i.test(file.name)||file.type==='application/vnd.openxmlformats-officedocument.presentationml.presentation'){const {parseZip,buildPresentation,RECOMMENDED_ZIP_LIMITS}=await loadPptxModule();const presentation=buildPresentation(await parseZip(buffer.slice(0),RECOMMENDED_ZIP_LIMITS),{lazySlides:true});pageCount=presentation.slides.length;if(!pageCount)throw Error("PPTX 안에서 표시할 슬라이드를 찾지 못했습니다.")}const safe=`${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"_")}`,path=`shared/${remote.user.id}/${safe}`;const {error:up}=await db.storage.from("class-materials").upload(path,file,{contentType:file.type||undefined});if(up)throw up;const {error}=await db.from("materials").insert({class_id:null,teacher_id:remote.user.id,kind:fd.get("kind"),title:String(fd.get("title")).trim(),description:String(fd.get("description")||"").trim(),file_path:path,page_count:pageCount});if(error){await db.storage.from("class-materials").remove([path]);throw error}})}
